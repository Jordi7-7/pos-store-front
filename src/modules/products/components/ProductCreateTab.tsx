...
import { Tag, Barcode, Check, Plus, Trash2, HelpCircle, X, Upload, Loader2 } from 'lucide-react';
...
export const ProductCreateTab: React.FC<ProductCreateTabProps> = ({
  ...
  // Generate dynamic list of all combinations (Cartesian Product)
  ...
  const handleCreateProductSubmit = async (e: React.FormEvent) => {
    ...
    if (!hasVariants) {
      ...
      if (variantsList.length === 0) {
        toast.error('Por favor agrega o genera al menos una variante al producto.');
        return;
      }
    ...
    try {
      ...
      console.error(err);
      toast.error('Error al registrar el producto en el servidor.');
    }
  };
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-fade-in relative">
      <div className="lg:col-span-2 space-y-6">
        ...
        <form onSubmit={handleCreateProductSubmit} className="bg-bg-card border border-border-card rounded-2xl p-6 space-y-5 shadow-sm">
          ...
          {/* SECTION A: Product WITHOUT variants */}
          {!hasVariants && (
          ...
          {/* SECTION B: Product WITH variants (Editable Grid) */}
          {hasVariants && variantsList.length > 0 && (
            <div className="border border-border-card rounded-xl p-4 bg-bg-dark/30 space-y-3 animate-fade-in overflow-hidden">
              <span className="text-[11px] font-bold text-secondary uppercase tracking-wider block">Variantes Agregadas ({variantsList.length})</span>
        ...
        {/* SECTION B (Builder Form): Display variant constructor */}
        {hasVariants && (
          <div className="bg-bg-card border border-border-card rounded-2xl p-6 space-y-5 shadow-sm animate-fade-in">
            <div className="border-b border-border-card pb-2">
              ...
              <p className="text-[11px] text-neutral mt-0.5">Elige qué atributos varían en este producto y agrega las combinaciones.</p>
            </div>

            {/* Activar / Desactivar Atributos */}
            <div className="bg-bg-dark/30 border border-border-card rounded-xl p-4 space-y-3.5">
              <div className="flex items-center justify-between">
                <span className="text-[11px] font-bold text-secondary uppercase tracking-wider block">Atributos a utilizar</span>
                <span className="text-[9px] text-neutral">Define la forma en que varía este producto</span>
              </div>

              {/* Add Active Attribute Selector */}
              {remainingAttributes.length > 0 && (
                <div className="flex gap-2">
                  <select
                    value={selectedAttributeToAdd}
                    onChange={(e) => setSelectedAttributeToAdd(e.target.value)}
                    className="flex-1 bg-bg-card border border-border-card rounded-lg px-2.5 py-1.5 text-xs text-secondary focus:outline-none"
                  >
                    <option value="">-- Seleccionar atributo para agregar --</option>
                  ...
                  <button
                    ...
                    className="bg-primary hover:bg-primary-hover text-white text-[11px] font-bold px-3 py-1.5 rounded-lg transition-colors disabled:opacity-50"
                  >
                    Agregar
                  </button>
                </div>
              ...
              {activeAttributeIds.length > 0 ? (
                <div className="flex flex-wrap gap-2 pt-1 border-t border-border-card/40 mt-1">
                  {activeAttributeIds.map(attrId => {
                    ...
                    return (
                      <div key={attrId} className="flex items-center gap-1.5 px-2.5 py-1 bg-bg-card border border-border-card rounded-lg text-xs text-secondary font-medium">
                        <span>{attr?.name}</span>
                        <button
                          type="button"
                ...
                <p className="text-[10px] text-neutral italic">No tienes atributos activos para este producto. Selecciona uno arriba para comenzar.</p>
              )}
            </div>

            {/* Generador Combinaciones Masivas & Constructor Manual */}
            {activeAttributeIds.length > 0 && (
      ...
      {/* Guide widget card */}
      <div className="bg-bg-card border border-border-card rounded-2xl p-6 shadow-sm h-fit space-y-4">
        <h4 className="text-xs font-bold text-secondary uppercase tracking-wide border-b border-border-card pb-2">Guía del Formulario</h4>
        <div className="text-xs text-neutral space-y-3.5">
          <p>
            1. **Imágenes**: Sube imágenes directamente usando el botón **"+ Subir Nueva Imagen"** en la sección de arriba.
          </p>
          <p>
            2. **Atributos y Variantes**: Activa qué atributos definen este producto, autogenera todas las combinaciones y luego ajusta precios, códigos o asocia fotos a nivel de variante en la grilla interactiva.
          </p>
          <div className="p-3 bg-bg-dark/40 border border-border-card/50 rounded-xl flex items-start gap-2">
            <HelpCircle className="w-4 h-4 text-primary shrink-0 mt-0.5" />
            <span className="text-[10px] leading-relaxed">
              El stock inicial ingresado se asignará de manera automática a la sucursal activa seleccionada en la cabecera.
            </span>
          </div>
        </div>
      </div>
